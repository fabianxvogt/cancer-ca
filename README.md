...
The current owner review keeps the dual-metric figure, rule-ablation studies, and
`local_division_rate` calibration open; see the [roadmap review](docs/roadmap-review-2026-08-25.md).
...
The values below are reported empirical outputs from `statistical_validation.py` within
this model class (deterministic coupling, narrow parameter regime; see the paper abstract
for scope). The historical alternative-metric sign-flip values are not fresh output from
the incomplete supplementary probe; the current comparison remains open in the
[owner review](docs/roadmap-review-2026-08-25.md).
...
├── figure_S1_metric_dependence.py  # Exploratory source: alternative-metric sign flip
...
| S1 exploratory metric-dependence source (not a committed figure) | `figure_S1_metric_dependence.py` |

All committed figures in `images/` were generated by these scripts; selected analyses
also have committed result JSONs beside their corresponding assets.
