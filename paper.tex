...
These simplifications limit biological realism but enable transparent analysis of the core evolutionary dynamics. All parameters, assumptions, and implementation details are documented for reproducibility (see Supplementary Table S1 for complete parameter set).
...
Parameters: $\theta_{\text{starve}} = 0.8$, $\theta_{\text{nutr}} = 0.05$, $c_{\text{cons}} = 0.005$, $r_{\text{regen}} = 0.05$, $\lambda_0 = 4.0$, $\delta_0 = 0.01$ for sensitive cells, $\delta_0 = 0.0005$ for resistant cells. See Supplementary Table S1 for complete parameter set with biological justifications.
...
\begin{figure}[h!]
\centering\includegraphics[width=0.95\textwidth]{images/figure6_parameter_space.png}
...
\begin{figure}[h!]
\centering\includegraphics[width=0.98\textwidth]{images/figure5_robustness.png}
...
To move beyond advocacy for either strategy, we propose a \textbf{conceptual phase diagram} (Figure~\ref{fig:phase_diagram}, schematic only---not computed from our model) showing where MTD might become superior to control-focused approaches:
...
\texttt{https://github.com/[username]/cancer-ca}
...
\textbf{Complete parameter specification:} Supplementary Table S1 (to be added) contains all model parameters with values, units, biological justifications, and literature sources. Key parameters: grid size (120$\times$120), initial tumor radius (20 cells), division rate $\lambda_0 = 4.0$, sensitive death rate $\delta_0 = 0.01$, resistant death rate $0.0005$, mutation rate $\mu_0 = 0.002$, therapy-induced mutagenesis $\alpha = 3.0$, nutrient consumption $0.005$, regeneration $0.05$. All simulations use same random seed structure for reproducibility.
